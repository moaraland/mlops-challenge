...
def prepare_dataset(
    ...
    examples, _ = tfds.load(
        dataset_name, with_info=True, as_supervised=True, try_gcs=True
    )
    ...
    n_train = write_tfrecord(
        train_ds, tokenizers, train_path, max_tokens, train_records
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--output_dir", required=True, help="Diretório de saída do dataset processado"
    )
    parser.add_argument(
        "--dataset_name", default="para_crawl/enpt", help="Dataset do TFDS"
    )
    parser.add_argument(
        "--max_tokens", type=int, default=64, help="Máximo de tokens por sentença"
    )
    parser.add_argument(
        "--train_records",
        type=int,
        default=5000,
        help="Número de exemplos de treino (subamostra)",
    )
    parser.add_argument(
        "--val_records",
        type=int,
        default=500,
        help="Número de exemplos de validação (subamostra)",
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="Seed de reprodutibilidade"
    )
